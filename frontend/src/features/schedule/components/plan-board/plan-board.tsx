...
export type PlanBoardMode = "init" | "adjust";
export type PlanBoardInitialData = {
  rooms: RoomRow[];
  batches: BatchRow[];
  teams: TeamRow[];
};
...
function computePlates(rooms: RoomRow[]): PlateRow[] {
  const out: PlateRow[] = [];
  const batchIds = Array.from(new Set(rooms.map(r => r.goLiveBatch))).filter(Boolean);
  for (const batchId of batchIds) {
    const batchRooms = rooms.filter(r => r.goLiveBatch === batchId);
    if (batchRooms.length === 0) continue;
    const meta = GO_LIVE_BATCHES.find(b => b.id === batchId);
    const index = batchIds.indexOf(batchId);
    const gxs = batchRooms.map(r => r.gx);
    const gys = batchRooms.map(r => r.gy);
    out.push({
      name: meta ? "批次 " + (GO_LIVE_BATCHES.indexOf(meta) + 1) : String(batchId),
      cust: batchRooms.map(r => r.code).join("+") + (meta ? " · 上线 " + meta.goLiveDate : ""),
      x0: Math.min(...gxs), y0: Math.min(...gys),
      x1: Math.max(...gxs), y1: Math.max(...gys),
      batchId,
      color: meta?.color ?? BATCH_COLORS[index % BATCH_COLORS.length]!,
    });
  }
  return out;
}
...
function roomDisplayCode(code: string): string {
  const suffix = code.match(/(\d{3})$/)?.[1];
  return suffix || code.replace(/\D/g, "") || code;
}
...
function planCardFromStrategy(option: StrategyPlan, index: number, total: number): Plan {
  ...
  const risks = [
    ...
    ...(option.risks ?? []).map(r => ({ lv: r.severity, t: r.risk_type, d: r.message })),
    ...(option.pulled_inputs ?? []).map(input => ({
      lv: "中",
      t: "前置输入提拉",
      d: `${input.target_desc} · ${input.current_date ?? "未定"} → ${input.suggested_date}`,
    })),
  ];
  return {
    id: option.option_id,
    ...
    kpi: [
      ...
      [String(option.kpis.total_duration_days), "总工期/天"],
      [signedNumber(-(option.kpis.compressed_days ?? 0)), "压缩/天", "cut"],
      [signedNumber(option.kpis.added_crew ?? 0), "新增/人", "add"],
    ],
    risks,
...
function strategyDescription(option: StrategyPlan): string {
  if (option.strategy === "均匀压缩") return "按关键路径活动共同承担压缩压力，形成当前可下发的稳定方案。";
  if (option.strategy === "集中压缩") return "优先压缩空间最大的关键活动，用更集中资源抢回目标节点。";
  if (option.strategy === "站货提拉") return "倒推机房就位 / 到货等前置输入，交业务跟上游锁定承诺。";
  return "目标时间有富余时延长 buffer，保留更稳的交付余量。";
}
...
function ganttRowsFromStrategy(option: StrategyPlan): GanttRow[] {
  const activities = option.plan.activities
    .filter(activity => activity.start_date && activity.end_date)
    .sort((a, b) => (a.start_date === b.start_date ? a.instance_id.localeCompare(b.instance_id) : a.start_date.localeCompare(b.start_date)));
  const picked = activities.filter(activity => activity.is_critical).slice(0, 7);
  const rows = (picked.length ? picked : activities.slice(0, 7));
  if (!rows.length) return GANTT_BASE;

  const minStart = rows.reduce((min, activity) => activity.start_date < min ? activity.start_date : min, rows[0]!.start_date);
  const maxEnd = rows.reduce((max, activity) => activity.end_date > max ? activity.end_date : max, rows[0]!.end_date);
  const span = Math.max(1, bwdDays(maxEnd, minStart) + 1);
  const scale = Math.min(1, 40 / span);

  return rows.map((activity) => {
    const start = Math.max(0, bwdDays(activity.start_date, minStart) * scale);
    const len = Math.max(1, (bwdDays(activity.end_date, activity.start_date) + 1) * scale);
    const compressed = Boolean(activity.standard_sla_days && activity.actual_sla_days < activity.standard_sla_days);
    return [shortActivityName(activity.activity_name), start, len, start, len, compressed];
  });
}

function shortActivityName(name: string): string {
  return name.replace(/^客户/, "").replace(/与验收$/, "").slice(0, 6);
}


/* ===================== 项目时间范围 & PoD 状态推演 · #10 ===================== */
const PROJECT_START = "2026-05-28";  // mock/兜底纪元：站货演示轴原点 + "可上架"无精确到货日时的代用日期；非进度"今天"，见 SCHED_NOW
...
// "在途 06-18" → "2026-06-18"；"可上架"（已到货但无精确日期）→ mock 纪元 PROJECT_START；"待定" → null
...
function applyParsedChangeSet(response: ParseChangesResponse, rooms: RoomRow[], batches: BatchRow[]) {
  const roomChanges = new Map((response.changes.rooms ?? []).map(room => [room.room_id, room]));
  const arrivalChanges = new Map((response.changes.arrivals ?? []).map(arrival => [arrival.pod_id, arrival]));
  const batchChanges = new Map((response.changes.batches ?? []).map(batch => [batch.batch_id, batch]));
  const touchedRooms = new Set<string>();
  const touchedPods = new Set<string>();
  const touchedBatches = new Set<string>();

  const nextRooms = rooms.map(room => {
    const roomChange = roomChanges.get(room.code);
    const nextPods = room.pods.map(pod => {
      const arrivalDate = arrivalChanges.get(pod.id)?.arrival_date;
      if (!arrivalDate) return pod;
      touchedPods.add(pod.id);
      return { ...pod, arrival: "eta" as const, etaLabel: "在途 " + arrivalDate.slice(5), etaDate: arrivalDate };
    });
    let nextRoom = room;
    if (roomChange) {
      const cableReadyAt = roomChange.cabling_ready_date ?? room.cableReadyAt;
      const equipReadyAt = roomChange.install_ready_date ?? room.equipReadyAt;
      const liquidReadyAt = roomChange.liquid_ready_date ?? room.liquidReadyAt;
      const hasReady = Boolean(cableReadyAt || equipReadyAt || liquidReadyAt);
      touchedRooms.add(room.code);
      nextRoom = {
        ...room,
        site: hasReady ? "ready" : room.site,
        readyUnknown: hasReady ? false : room.readyUnknown,
        cableReadyAt,
        equipReadyAt,
        liquidReadyAt,
      };
    }
    return nextRoom !== room || nextPods.some((pod, index) => pod !== room.pods[index])
      ? { ...nextRoom, pods: nextPods }
      : room;
  });

  const nextBatches = batches.map(batch => {
    const batchChange = batchChanges.get(batch.id);
    if (!batchChange) return batch;
    touchedBatches.add(batch.id);
    return {
      ...batch,
      powerOnDate: batchChange.power_on_target_date ?? batch.powerOnDate,
      goLiveDate: batchChange.online_target_date ?? batch.goLiveDate,
    };
  });

  const keys = [
    ...Array.from(touchedRooms).map(id => "room:" + id),
    ...Array.from(touchedPods).map(id => "pod:" + id),
    ...Array.from(touchedBatches).map(id => "batch:" + id),
  ];
  const parts = [
    touchedRooms.size ? `${touchedRooms.size} 间机房 ready` : null,
    touchedPods.size ? `${touchedPods.size} 个 PoD 到货日期` : null,
    touchedBatches.size ? `${touchedBatches.size} 个批次目标` : null,
  ].filter(Boolean) as string[];

  return { rooms: nextRooms, batches: nextBatches, keys, parts };
}

function parseWarningSuffix(warnings: string[]): string {
  if (!warnings.length) return "";
  const more = warnings.length > 1 ? `（另 ${warnings.length - 1} 条）` : "";
  return ` · 提醒：${warnings[0]}${more}`;
}
...
// 把 ISO 日期字符串和总天数互转，用于站货 mock 指针滑块
...
function UploadModal({ onClose }: { onClose: () => void }) {
  ...
  const addFiles = (list: FileList | null) => {
    ...
    const file = Array.from(list).find(f => /\.xlsx$/i.test(f.name)) ?? list[0]!;
    setFiles([{ file, name: file.name, size: fmtSize(file.size) }]);
  };
  const download = () => {
    setDownloading(true);
    void downloadChangeTemplate()
      .catch(error => console.warn("[T-013] change template download failed", { error }))
      .finally(() => setDownloading(false));
  };
  const parse = () => {
    if (!files.length) return;
    window.dispatchEvent(new CustomEvent('aida:plan-ingest', { detail: { files: files.map(f => f.file), names: files.map(f => f.name) } }));
    onClose();
  };
  return (
    <div className="up-mask" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="up-modal" role="dialog" aria-modal="true">
        ...
        <div className={"up-drop" + (dragOver ? " over" : "")}
          ...
          <div className="up-drop-sub">固定模板 Excel · 机房 ready / 到货 / 批次目标</div>
        ...
        <div className="up-foot">
          <div className="up-actions">
            <button className="up-btn" disabled={downloading} onClick={download}>{downloading ? "下载中..." : "下载模板"}</button>
...
function ClawReportEntry({ report, onOpen }: { report: AdjustmentReport; onOpen: () => void }) {
  return (
    <div className="ar-entry">
      <div className="ar-entry-head">
        <span className="ar-entry-ic">{I.doc(12)}</span>
        <span className="ar-entry-title">调整报告</span>
        <span className="ar-entry-ver">{report.committedPlanVersion}</span>
      </div>
      <div className="ar-entry-body">
        <span>{report.strategyLabel}</span>
        <b>{report.kpis.find(kpi => kpi.label === '总工期')?.value ?? '已下发'}</b>
      </div>
      <button className="ar-entry-btn" onClick={onOpen}>查看调整报告</button>
    </div>
  );
}

function AdjustmentReportModal({ report, onClose }: { report: AdjustmentReport; onClose: () => void }) {
  const hasAiSummary = Boolean(report.aiSummary?.trim());
  const [exporting, setExporting] = useState(false);
  const exportPlan = () => {
    if (exporting) return;
    setExporting(true);
    void downloadDeliveryPlan({ planId: report.planId, version: report.version })
      .then(() => console.info("[T-021] delivery plan exported", { plan_id: report.planId, version: report.version }))
      .catch(error => console.warn("[T-021] delivery plan export failed", { error }))
      .finally(() => setExporting(false));
  };
  return (
    <div className="ar-mask" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="ar-modal" role="dialog" aria-modal="true" aria-label="调整报告">
        <div className="ar-head">
          <div>
            <div className="ar-title">{I.doc(14)} {report.title}</div>
            <div className="ar-sub">{report.modeLabel} · {report.strategyLabel} · {report.committedPlanVersion}</div>
          </div>
          <button className="ar-close" onClick={onClose} aria-label="关闭">×</button>
        </div>
        <div className="ar-body">
          <section className="ar-section ar-summary">
            <div className="ar-section-title">本次结论</div>
            <div className="ar-summary-grid">
              <div>
                <span>选定策略</span>
                <b>{report.strategyLabel}</b>
              </div>
              <div>
                <span>建议</span>
                <b>{report.advice}</b>
          ...
          <section className="ar-kpis">
            {report.kpis.map((kpi) => (
              <div className={`ar-kpi ${kpi.tone ?? ''}`} key={kpi.label}>
                <span>{kpi.label}</span>
                <b>{kpi.value}</b>
              </div>
            ))}
          ...
          {report.notes.length > 0 && (
            <section className="ar-section">
              <div className="ar-section-title">解释要点</div>
              <div className="ar-note-list">
                {report.notes.map((note, index) => <span key={index}>{note}</span>)}
              </div>
            </section>
          )}

          <section className="ar-section">
            <div className="ar-section-title">活动顺延 / 提前</div>
            {report.movedActivities.length > 0 ? (
              <div className="ar-move-list">
                {report.movedActivities.map(activity => (
                  <div className="ar-move" key={activity.instanceId}>
                    <div className="ar-move-main">
                      <b>{activity.name}</b>
                      <span>{activity.instanceId}</span>
                    </div>
                    <div className="ar-move-dates">
                      <span>{activity.oldStart && activity.oldEnd ? `${activity.oldStart} → ${activity.oldEnd}` : '旧日期未返回'}</span>
                      <i>{activity.direction}</i>
                      <span>{activity.newStart} → {activity.newEnd}</span>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="ar-empty">本次下发未返回单活动顺延 / 提前明细。</div>
            )}
          </section>

          <section className="ar-section">
            <div className="ar-section-title">风险清单</div>
            {report.risks.length > 0 ? (
              <div className="ar-risk-list">
                {report.risks.map((risk, index) => (
                  <div className="ar-risk" key={index}>
                    <span className={"ar-sev " + (risk.severity === '高' ? 'hi' : risk.severity === '中' ? 'mid' : 'lo')}>{risk.severity}</span>
                    <div>
                      <b>{risk.riskType}</b>
                      <span>{risk.message}</span>
                      {risk.mitigation && <i>{risk.mitigation}</i>}
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="ar-empty">本次方案未返回新增风险明细。</div>
            )}
          </section>

          <section className="ar-section">
            <div className="ar-section-title">未满足项</div>
            {report.unmet.length > 0 ? (
              <div className="ar-unmet-list">
                {report.unmet.map((item, index) => (
                  <div className="ar-unmet" key={index}>
                    <b>{item.target}</b>
                    <span>{item.reason}{item.gapDays != null ? ` · 差 ${item.gapDays} 天` : ''}</span>
                  </div>
                ))}
              </div>
            ) : (
              <div className="ar-empty">当前选定方案无未满足项。</div>
            )}
          </section>

          {hasAiSummary && (
            <section className="ar-section">
              <div className="ar-section-title">AI 总结</div>
              <div className="ar-ai">{report.aiSummary}</div>
            </section>
          )}
        </div>
        <div className="ar-foot">
          <span>计划 {report.planId}</span>
          <div className="ar-foot-actions">
            <button className="up-btn" disabled={exporting} onClick={exportPlan}>{exporting ? "导出中..." : "导出交付计划表"}</button>
            <button className="up-btn pri" onClick={onClose}>关闭</button>
          </div>
        </div>
      </div>
    </div>
...
type ClawStats = { group: string; rooms: number; pods: number; teams: number; headcount: number };
function ScheduleClawRail({ width, side, stats, adjustmentReport, onOpenAdjustmentReport, onResize, onSwap }: {
  width: number;
  side: 'left' | 'right';
  stats: ClawStats;
  adjustmentReport?: AdjustmentReport | null;
  onOpenAdjustmentReport: () => void;
  ...
  return (
    ...
    <aside className="claw-rail" style={{ flexBasis: width, width }}>
      ...
      <div className="cr-body">
        ...
        {adjustmentReport && <ClawReportEntry report={adjustmentReport} onOpen={onOpenAdjustmentReport} />}
        ...
        {sent && (<>
          ...
          <div className="msg fade"><div className="who">Claw</div>
            <div className="bubble ai">已接收 5 份资料，识别 <span className="num hi">{stats.rooms}</span> 机房 / <span className="num hi">{stats.pods}</span> PoD / <span className="num hi">{stats.teams}</span> 队伍 · <span className="num hi">{stats.headcount}</span> 人。正在按 <b>站 → 货 → 人</b> 解析，并构建初排方案…</div></div>
        </>)}
      </div>
...
// 站货演示窗口仍使用 mock PROJECT_START/PROJECT_END；排期倒排图的真数据轴走 BackwardSchedule.axisOf。
  ...
  return (
    <div className={"room "+(inView?"in ":"")+(picked?"picked ":"")+(faint?"faint ":"")+(inRange?"":"out-of-range")}
      ...
      <div className="rname">
        ...
        <span className="rname-txt">机房{roomDisplayCode(r.code)}</span>
...
function BatchManager({ batches, rooms, onAssign, onCreate, onUpdate, onDelete }: {
  ...
  const podRoom = useMemo(() => {
    ...
    rooms.forEach(r => r.pods.forEach(p => { m[p.id] = "机房" + roomDisplayCode(r.code); }));
    return m;
  }, [rooms]);
...
function axisOf(rows: BwdRow[]) {
  ...
  if (!ds.length) return { min: PROJECT_START, max: PROJECT_END, span: Math.max(1, bwdDays(PROJECT_END, PROJECT_START)) }; // 无行时才落回 mock 轴
...
function bwdRowsFromPlan(batches: BatchRow[], rooms: RoomRow[], sug: boolean): BwdRow[] {
  ...
  return batches
    ...
    .map(b => {
      ...
      if (sug) {
        arrive = bwdShift(b.goLiveDate, -MOCK_LIVE_TO_ARRIVE_DAYS);
        room = bwdShift(b.goLiveDate, -MOCK_LIVE_TO_ROOM_DAYS);
      } else {
        arrive = allArrived
          ? bwdShift(b.goLiveDate, -MOCK_LIVE_TO_ARRIVE_DAYS)
          : (maxDate(pods.map(id => { const p = podInfo[id]?.pod; return p ? (p.etaDate || parseEtaToDate(p.etaLabel)) : null; })) || bwdShift(b.goLiveDate, -MOCK_LIVE_TO_ARRIVE_DAYS));
        const roomMap: Record<string, RoomRow> = {}; pods.forEach(id => { const rm = podInfo[id]?.room; if (rm) roomMap[rm.code] = rm; });
        room = maxDate(Object.values(roomMap).flatMap(rm => [rm.cableReadyAt, rm.equipReadyAt, rm.liquidReadyAt])) || bwdShift(b.goLiveDate, -MOCK_LIVE_TO_ROOM_DAYS);
      }
...
function mapGenerateResponseToBwdRows(response: GenerateResponse, batches: BatchRow[], rooms: RoomRow[]): GeneratedBwdRow[] {
  const baseRows = bwdRowsFromPlan(batches, rooms, true);
  const suggestions = new Map((response.readiness_suggestions ?? []).map((item) => [item.batch_id, item]));
  const roomByPod = new Map<string, string>();
  rooms.forEach((room) => room.pods.forEach((pod) => roomByPod.set(pod.id, room.code)));

  return baseRows.map((row) => {
    const suggestion = suggestions.get(row.id);
    const podSet = new Set(row.pods);
    const roomSet = new Set(row.pods.map((podId) => roomByPod.get(podId)).filter((roomId): roomId is string => !!roomId));
    const arriveActs = response.plan.activities.filter(
      (activity) => activity.milestone_kind === '到货' && activity.scope_ref.scope === 'PoD级' && podSet.has(activity.scope_ref.ref_id ?? ''),
    );
    const roomActs = response.plan.activities.filter(
      (activity) => activity.milestone_kind === '机房就位' && activity.scope_ref.scope === '机房级' && roomSet.has(activity.scope_ref.ref_id ?? ''),
    );
    const powerAct = latestActivity(
      response.plan.activities.filter(
        (activity) => activity.milestone_kind === '上电' && activity.scope_ref.scope === '批次级' && activity.scope_ref.ref_id === row.id,
      ),
    );
    const onlineAct = latestActivity(
      response.plan.activities.filter(
        (activity) => activity.milestone_kind === '上线' && activity.scope_ref.scope === '批次级' && activity.scope_ref.ref_id === row.id,
      ),
    );
    const suggestedArrival = maxDate(Object.values(suggestion?.suggested_arrival ?? {}));
    const suggestedRoom = maxDate(Object.values(suggestion?.suggested_room_ready ?? {}));

    return {
      ...row,
      arrive: maxDate(arriveActs.map((activity) => activity.end_date)) ?? suggestedArrival ?? row.arrive,
      room: maxDate(roomActs.map((activity) => activity.end_date)) ?? suggestedRoom ?? row.room,
      powerOnDate: powerAct?.end_date ?? row.powerOnDate,
      goLiveDate: onlineAct?.end_date ?? row.goLiveDate,
      ai: {
        arrive: arriveActs.some((activity) => activity.is_ai_generated) || Boolean(suggestedArrival),
        room: roomActs.some((activity) => activity.is_ai_generated) || Boolean(suggestedRoom),
      },
    };
  }).sort((a, b) => (a.goLiveDate < b.goLiveDate ? -1 : 1));

  function latestActivity(activities: ScheduledActivity[]): ScheduledActivity | null {
    return activities.reduce<ScheduledActivity | null>((latest, activity) => {
      if (latest === null) return activity;
      return activity.end_date > latest.end_date ? activity : latest;
    }, null);
  }

  function maxDate(dates: string[]): string | null {
    return dates.length ? dates.reduce((latest, date) => (date > latest ? date : latest), dates[0]!) : null;
  }
}
...
function BackwardSchedule({ batches, rooms, updateBatch, onConfirm, variant = "suggest", onPickBatch, showConfirm = true, confirmTitle = "确认上线 / 上电目标", confirmDesc = "系统据此倒推建议到货 / 机房就位，并生成 A / B / C 多策略排期方案", confirmLabel = "确认目标 · 生成排期方案 →", events = [], onAddEvent, onRemoveEvent, generatedRows, onConfirmSuggestion, confirmedSuggestions }: {
  ...
  const dispRows: GeneratedBwdRow[] = sug ? allRows : allRows.map(r => { const f = baseRef.current?.[r.id]; return f ? { ...r, arrive: f.arrive, room: f.room } : r; });
  ...
  const latestArrive = dispRows.reduce((m, r) => (r.arrive > m ? r.arrive : m), dispRows[0]!.arrive);
  const latestRoom = dispRows.reduce((m, r) => (r.room > m ? r.room : m), dispRows[0]!.room);
  ...
  const beginDrag = (e: React.MouseEvent, row: BwdRow, field: "powerOnDate" | "goLiveDate") => {
    ...
    const onMove = (mv: MouseEvent) => {
      ...
      if (field === "goLiveDate") {
        if (row.powerOnDate) {
          const lo = bwdShift(row.powerOnDate, 1);
          if (nd < lo) nd = lo;
        }
      } else {
  ...
  return (
    <div className="bwd">
      ...
      <div className="bwd-banner fade">
        <div className={"bwd-bx" + (sug ? " sug" : "")}><div className="lab">机房最晚 ready</div><div className="val">{bwdMD(latestRoom)}</div></div>
        <div className={"bwd-bx" + (sug ? " sug" : "")}><div className="lab">货最晚到</div><div className="val">{bwdMD(latestArrive)}</div></div>
        <div className="bwd-mid"><span className="bwd-arrow">←</span><div className="gd">上线前置期 · {liveToArriveDays} 天</div></div>
              ...
              <div className="be-form-body">
                ...
                <label className="be-fld">
                  ...
                  <span className="be-dates">
                    <input className="be-date" type="date" min={axis.min} max="2026-12-31" value={evDraft.start}
                      onChange={e => setEvDraft(d => d && { ...d, start: e.target.value })} />
                    <i>~</i>
                    <input className="be-date" type="date" min={axis.min} max="2026-12-31" value={evDraft.end}
...
type GeneratedScheduleView = { acts: GdAct[]; criticalIds: Set<string>; rows: GeneratedBwdRow[] };

function normalizeDurationOverrides(overrides: DurationOverrides): DurationOverrides {
...
function replaceOptionPlan(response: AdjustResponse, optionId: string, plan: PlanResult): AdjustResponse {
  return {
    ...response,
    options: response.options.map((option) =>
      option.option_id === optionId ? { ...option, plan } : option,
    ),
  };
}
...
function mapGenerateResponseToGantt(response: GenerateResponse, batches: BatchRow[], rooms: RoomRow[]): GeneratedScheduleView {
  ...
  const acts = response.plan.activities.map((activity) => {
    ...
    return {
      ...
      deps: activity.predecessor_instance_ids ?? [],
      progress: 0,
  ...
  return {
    acts,
    criticalIds: new Set(response.plan.critical_path ?? []),
    rows: mapGenerateResponseToBwdRows(response, batches, rooms),
  };

  function batchForActivity(activity: ScheduledActivity): BatchRow | undefined {
    const refId = activity.scope_ref.ref_id ?? '';
    if (activity.scope_ref.scope === '批次级') return batchById.get(refId);
    if (activity.scope_ref.scope === 'PoD级') return batchByPod.get(refId);
    if (activity.scope_ref.scope === '机房级') return batchByRoom.get(refId);
    return undefined;
  }

  function phaseForActivity(activity: ScheduledActivity): string {
    if (activity.milestone_kind === '机房就位') return 'dc';
    if (activity.milestone_kind === '到货') return 'goods';
    if (activity.milestone_kind === '上电' || activity.milestone_kind === '上线') return 'power';
    if (activity.milestone_kind === '移交') return 'accept';
...
function GanttDetailOverlay({ title, backLabel = "← 返回", onClose, onSaved, batches, rooms, events, acts, criticalIds, initialDurationOverrides = {} }: { title: string; backLabel?: string; onClose: () => void; onSaved: (overrides: DurationOverrides) => void; batches: BatchRow[]; rooms: RoomRow[]; events: SchedEvent[]; acts?: GdAct[]; criticalIds?: Set<string>; initialDurationOverrides?: DurationOverrides }) {
  ...
  const [durations, setDurations] = useState<DurationOverrides>(() => initialDurationOverrides);
  ...
  const base = useMemo(() => {
    ...
    gdActs.forEach(a => { start[a.id] = a.start; end[a.id] = a.end; dur[a.id] = a.milestone ? 0 : GD.between(a.start, a.end); });
    gdActs.forEach(a => {
      if (!a.deps.length) { lag[a.id] = null; return; }
      let mx: Date | null = null; a.deps.forEach(d => { const depEnd = end[d]; if (!depEnd) return; const e = GD.parse(depEnd); if (!mx || e > mx) mx = e; });
      lag[a.id] = mx ? GD.diff(GD.add(mx, 1), GD.parse(a.start)) : null;
    });
  ...
  const affectedActs = useMemo(() => {
    ...
    gdActs.forEach(a => { if (a.milestone) return; const as = eff.start[a.id] || a.start, ae = eff.end[a.id] || a.end; if (events.some(ev => as <= ev.end && ae >= ev.start)) s.add(a.id); });
    return s;
  }, [events, eff, gdActs]);
  ...
  const focusGanttAct = useCallback((id: string) => {
    ...
    requestAnimationFrame(() => {
      ...
      const act = gdActs.find((x) => x.id === id);
  ...
  const goId = latestOnlineId ?? finalActId;
  const baseGo = goId ? base.start[goId]! : PROJECT_START; // 无甘特活动时才落回 mock 纪元
  const projGo = goId ? (eff.start[goId] || baseGo) : baseGo;
  ...
  const criticalNodeIds = Array.from(critIds);
  const isMockCriticalChain = ["G00", "G01", "B3A", "B3I", "B3C", "B3P", "B3U", "B3G", "G99", "G98"]
    .every((id, index, chain) => critIds.has(id) && criticalNodeIds[index] === id && chain.length === criticalNodeCount);
  const criticalPathSummary = isMockCriticalChain ? "批次3 链路最紧" : "当前链路最紧";

  function latestActId(items: GdAct[]): string | null {
    if (!items.length) return null;
    return items.reduce((latest, item) => (item.end > latest.end ? item : latest), items[0]!).id;
  }
    ...
    <div className="gd-panel gd-inline">
        <div className="gd-head">
          <div className="gd-head-l">
            ...
            <span className="gd-sub">单项目 · {rooms.length} 机房 / {totalPods} PoD / {batches.length} 批 · 拖活动条右端或点天数改工期,下游自动顺延</span>
          </div>
        ...
        <div className="gd-kpis">
          <div className={"gd-kpi" + (goDelta > 0 ? " late" : goDelta < 0 ? " early" : "")}>
            ...
            <div className="gd-kpi-d">{goDelta === 0 ? `= 基线 ${baseGo.slice(5)}` : (goDelta > 0 ? `晚 ${goDelta} 天` : `早 ${-goDelta} 天`)} · 目标 {baseGo.slice(5)}</div>
          </div>
          ...
          <div className={"gd-kpi" + (showCrit ? " gd-kpi-crit-on" : "")}>
            ...
            <div className="gd-kpi-v">{criticalNodeCount}<span className="u"> 节点</span></div>
            <div className="gd-kpi-d">{showCrit ? `${criticalPathSummary} · 下方时间轴已展开` : "勾选右侧「关键路径」展开时间轴"}</div>
          </div>
          <div className="gd-kpi">
        ...
        {events.length > 0 && (
          <div className="gd-exc-bar readonly">
            ...
            {events.map(ev => {
              const n = gdActs.filter(a => !a.milestone && (eff.start[a.id] || a.start) <= ev.end && (eff.end[a.id] || a.end) >= ev.start).length;
              return (
              ...
              return (
                <div className={"adj-item"+(openKey===k?" open":"")+(ready?"":" gap")} key={k}>
                  <button className="adj-item-h" onClick={()=>toggle(k)}>
                    <span className="adj-code">机房{roomDisplayCode(r.code)}</span>
...
function PlanBoard({ mode = "init", initialData }: { mode?: PlanBoardMode; initialData?: PlanBoardInitialData }){
  ...
  // 站货演示窗口默认用 mock PROJECT_START/PROJECT_END；排期真轴不读这里。
  ...
  const [rooms, setRooms] = useState<RoomRow[]>(() => initialData?.rooms ?? (mode === "adjust" ? INITIAL_ROOMS : INITIAL_ROOMS_BARE));
  const [teams, setTeams] = useState<TeamRow[]>(() => initialData?.teams ?? INITIAL_TEAMS);
  // 批次（PoD 级）state · 用户决策 2026-05-29：detail「批次」tab 拖拽编辑（步骤①，画布暂未联动）
  const [batches, setBatches] = useState<BatchRow[]>(() => initialData?.batches ?? INITIAL_BATCHES);
  ...
  useEffect(() => {
    if (initialData) {
      console.info("[T-022] PlanBoard mounted with resolved /plan data", {
        ...
        pods: initialData.rooms.reduce((sum, room) => sum + room.pods.length, 0),
        batches: initialData.batches.length,
        teams: initialData.teams.length,
      });
      return;
    }
    const controller = new AbortController();
    void loadProjectData({ signal: controller.signal }).then((result) => {
      ...
      if (result.source === "api") {
        ...
        console.info("[T-019] /project-data api connected", {
          project_id: result.data.project.project_id,
          rooms: result.data.rooms.length,
          pods: result.data.rooms.reduce((sum, room) => sum + room.pods.length, 0),
          batches: result.data.batches.length,
          teams: result.data.teams.length,
        });
      } else {
        console.warn("[T-019] /project-data fallback to mock", { reason: result.reason, error: result.error });
      }
    });
  ...
  const [durationOverridesByOption, setDurationOverridesByOption] = useState<Record<string, DurationOverrides>>({});
  ...
  const selectedAdjustSchedule = useMemo(() => {
    if (!selectedAdjustOption) return null;
    return mapGenerateResponseToGantt({
      plan: selectedAdjustOption.plan,
  ...
  const runGenerateSchedule = useCallback((input: { rooms: RoomRow[]; batches: BatchRow[]; teams: TeamRow[] }, logTag: string) => {
    ...
    void generateInitialSchedule(input).then((result) => {
      ...
      if (result.source === "api") {
        ...
        console.info(`${logTag} /generate api connected`, { plan_version: result.response.plan.version });
      } else {
        setGenerateStatus(result.reason === "forced" ? "mock" : "fallback");
        console.warn(`${logTag} /generate fallback to mock`, { reason: result.reason, error: result.error });
      }
    });
  }, []);
  ...
  const runDispatch = (id: string) => {
    ...
    const optionForReport = adjustResponse?.options.find(option => option.option_id === id) ?? null;
    const durationOverrides = durationOverridesByOption[id];
    const durationOverrideCount = Object.keys(durationOverrides ?? {}).length;
    const showToast = (text: string) => { setToast(text); setTimeout(() => setToast(null), 2800); };
    if (adjustStatus === "api") {
      setAdjustmentReport(null);
      setAdjustmentReportOpen(false);
      void commitSchedule({ rooms, batches, teams, optionId: id, durationOverrides }).then((result) => {
        if (result.source === "api") {
          if (adjustResponse && optionForReport) {
            setAdjustmentReport(buildAdjustmentReport({
              adjustResponse,
              selectedOption: optionForReport,
              commitResponse: result.response,
              previousPlan: optionForReport.plan,
              durationOverrideCount,
            }));
          }
          setAdjustResponse(prev => prev ? replaceOptionPlan(prev, id, result.response.plan) : prev);
          setDurationOverridesByOption(prev => {
            const next = { ...prev };
            delete next[id];
            return next;
          });
          console.info("[T-016] /commit api connected", { plan_id: result.response.plan_id, new_version: result.response.new_version, duration_override_count: durationOverrideCount });
          showToast(`已确认并下发执行：${p?.name ?? "该方案"} · v${result.response.new_version} · 通知 ${teams.length} 支队伍进场`);
        } else {
          console.warn("[T-016] /commit fallback to mock", { reason: result.reason, error: result.error });
          showToast(`已确认并下发执行：${p?.name ?? "该方案"} · 通知 ${teams.length} 支队伍进场`);
        }
      });
      return;
    }
  ...
  const saveGanttDurationOverrides = (overrides: DurationOverrides) => {
    const normalized = normalizeDurationOverrides(overrides);
    const optionId = gantt?.optionId;
    if (optionId) {
      setDurationOverridesByOption(prev => {
        const next = { ...prev };
        if (Object.keys(normalized).length) next[optionId] = normalized;
        else delete next[optionId];
        return next;
      });
      console.info("[T-016] duration overrides saved for commit", { option_id: optionId, duration_override_count: Object.keys(normalized).length });
    }
    setGantt(null);
    if(ganttReturn!=null){ setSel(ganttReturn===activeStep?null:ganttReturn); setGanttReturn(null); }
    setToast("排期已写回系统");
  ...
  const runIngest = (files: File[]) => {
    ...
    void parseChangeTemplate({ file, rooms, batches, teams }).then(result => {
      ...
      const applied = applyParsedChangeSet(result.response, rooms, batches);
      const warnings = result.response.warnings ?? [];
      if (applied.keys.length) {
        setRooms(applied.rooms);
        setBatches(applied.batches);
        setChanges(s => {
          const next = new Set(s);
          applied.keys.forEach(key => next.add(key));
          return next;
        });
        setAdjustResponse(null);
        setDurationOverridesByOption({});
    ...
    if (!isAdjust) {
      ...
      void ensureScheduleBaseline({ rooms, batches, teams }).then((result) => {
        if (result.source === "api") {
          ...
          setAdjustStatus(result.reason === "forced" ? "mock" : "fallback");
          console.warn("[T-011] /generate baseline fallback to mock", { reason: result.reason, error: result.error });
        }
      });
    }
  ...
  useEffect(()=>{ const h=()=>start(); window.addEventListener('aida:claw-send', h); return ()=>window.removeEventListener('aida:claw-send', h); },[start]);
  ...
  const statGroup = rooms[0]?.proj ?? BATCH.group;
  ...
  return (
    <div className={`planinit${clawSide === 'right' ? ' sched-cr-right' : ''}`}>
      <ScheduleClawRail
        width={clawWidth}
        side={clawSide}
        stats={{ group: statGroup, rooms: statRooms, pods: statPods, teams: statTeams, headcount: statHeadcount }}
        adjustmentReport={adjustmentReport}
        onOpenAdjustmentReport={() => setAdjustmentReportOpen(true)}
        onResize={setClawWidth}
        onSwap={() => setClawSide(s => (s === 'left' ? 'right' : 'left'))}
      />
      <div className="main">
        <div className="topbar">
          ...
          {started && <div className="tb-right">
            ...
            <button className="ctrl-btn ctrl-gantt" title="查看当前计划 · 整体排期甘特" onClick={()=>{ setGanttReturn(view); setSel(4); setGantt({ title:"整体排期 · 全量甘特", back:"← 返回", optionId: selectedAdjustOption?.option_id }); }}>{I.schedule(13)}当前计划</button>
      ...
      {adjustmentReportOpen && adjustmentReport && <AdjustmentReportModal report={adjustmentReport} onClose={() => setAdjustmentReportOpen(false)} />}
