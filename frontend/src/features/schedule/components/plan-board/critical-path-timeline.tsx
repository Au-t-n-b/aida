...
const MOCK_CP_CHAIN = ["G00", "G01", "B3A", "B3I", "B3C", "B3P", "B3U", "B3G", "G99", "G98"];
...
function buildNodes(
  ...
  return Array.from(criticalIds).filter((id) => acts.has(id) && eff.start[id] && eff.end[id] && base.start[id] && base.end[id]).map((id) => {
    const a = acts.get(id)!;
...
export function CriticalPathTimeline({
  ...
  const nodes = useMemo(() => buildNodes(actMap, criticalIds, eff, base, diff), [actMap, criticalIds, eff, base, diff]);
  const isMockCriticalChain = useMemo(
    () => nodes.length === MOCK_CP_CHAIN.length && nodes.every((node, index) => node.id === MOCK_CP_CHAIN[index]),
    [nodes],
  );
  const chainLabel = isMockCriticalChain ? "批次3 主链路" : "当前链路";

  const { totalDays, totalDelta, startIso, endIso } = useMemo(() => {
    const first = nodes[0], last = nodes[nodes.length - 1];
    if (!first || !last) return { totalDays: 0, totalDelta: 0, startIso: "", endIso: "" };
    const startIso = eff.start[first.id] || base.start[first.id]!;
    const endIso = eff.end[last.id] || eff.start[last.id] || base.end[last.id] || base.start[last.id]!;
    const bStart = base.start[first.id]!;
    const bEnd = base.end[last.id] || base.start[last.id]!;
      ...
      <div className="cp-head">
        <div className="cp-head-l">
          ...
          <span className="ro-sub">
            {chainLabel} / <b className="tnum">{nodes.length}</b> 节点
          </span>
          <span className="ro-sub">
