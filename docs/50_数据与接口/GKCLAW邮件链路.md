...
zhgk(backagent·内网) → mailgw POST /api/send → 邮件[task.dispatch ZIP] → frontagent(公网)
        ↑                                                                    ↓ App 现场工勘
  自动通知/手动刷新检查 ← mailgw POP3/API inbox ← 邮件[import_ack/result/error ZIP]
```
...
- 流水线接点：`confirm_table → task_dispatch/resurvey_gate(HITL·下发方式) → wait_survey(双源)`。
...
部署清单、部署后自验、联调前信息交换、十步联调（含每步验证点）、排障速查与回退方案，
统一见 **[GKCLAW部署与联调指南.md](GKCLAW部署与联调指南.md)**（单一真相，本文不复制）。
部署/换机前逐项核对变量，见 **[GKCLAW配置变量清单.md](GKCLAW配置变量清单.md)**。
...
仅 survey_work 与复勘轮走 GKCLAW 下发；supplement 已复用复勘闭环，不单独派生第三条链路。
自动回传依赖 mailgw `pop3.poll_interval` + `agent_notify`，也保留等待页手动刷新；evidence 不回填表内图片列；单 mailgw ↔ 单 AIDA 实例
