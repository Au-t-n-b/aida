...
class OpenAICompatProvider(LLMProvider):
    ...
    def _build_client(self) -> None:
        ...
        ssl_env = os.environ.get("ZHIPU_SSL_VERIFY", "").strip().lower()
        if ssl_env in ("1", "true", "yes"):
            verify = True
        elif ssl_env in ("0", "false", "no"):
            verify = False
        else:
        ...
        elif proxy or not verify:
            # 企业代理 MITM：与 AIDA agent/llm.py 一致，显式 proxy + verify
            http_client = httpx.AsyncClient(
                proxy=proxy,
                verify=verify,
                timeout=timeout_s,
