"""Load l3_skill_index.yaml from subskills or fallback agent-skill_full1."""
...
def l3_index_path(capability_root: Path) -> Path:
    return capability_root / "lld-dispatch-orchestrator.code1" / "l3_skill_index.yaml"


def load_l3_index(capability_root: Path) -> Dict[str, L3SkillDef]:
    path = l3_index_path(capability_root)
...
def resolve_package_dir(capability_root: Path, package: str) -> Path:
    pkg = package.replace("\\", "/").lstrip("./")
    while pkg.startswith("../"):
        pkg = pkg[3:]
    return (capability_root / pkg).resolve()
