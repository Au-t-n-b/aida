...
def validate_connect(path: Path, sheet: object) -> List[str]:
    ...
    try:
        from cc_glm_io import find_header_row
    ...
    data = raw.iloc[h + 1 :]
    if data.empty:
        errors.append("connect: no data rows after header")
        return errors
    if data.shape[1] < 3:
        errors.append("connect: need >= 3 columns")
        return errors
    pattern = "|".join(map(re.escape, STORAGE_NAME_KEYWORD))
    first_col = data.iloc[:, 0].astype(str)
    storage_rows = first_col.str.contains(pattern, na=False).sum()
    if storage_rows == 0:
        errors.append(
            f"connect: no rows matching STORAGE_NAME_KEYWORD {STORAGE_NAME_KEYWORD}"
        )
    print(
        f"connect: OK — sheet={resolved!r}, header_row={h}, "
        f"data_rows={len(data)}, storage_rows={storage_rows}"
    )
    return errors
...
def validate_resource(path: Path, sheet_index: int) -> List[str]:
    ...
    if not errors:
        try:
            layer, gw_pos, rule = detect_cc_glm_layer(path, sheet_index)
            print(
...
def main() -> int:
    p = argparse.ArgumentParser(description="Validate A3 存储管理面 IP inputs")
