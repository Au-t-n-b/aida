"""存储管理面网段规划规则（对齐 a3_i2 / a3_i3 prompt，IP 需求 = 节点数 × 8）。"""
...
from dw_manage_segment_rules import (  # noqa: F401 — shared with dw skill pattern
...
def plan_switch_gateways_glm_i2(
    *,
    ...
    """I2：多 Leaf 可共用网段；累加维度为 IP 数量（节点数×8）。"""
    if not switch_list:
        return [], 0, 0
...
def switch_gateways_to_dataframe(
    ...
    """对齐 display_message 0-6 网段规划 Markdown 表列。"""
    import pandas as pd
