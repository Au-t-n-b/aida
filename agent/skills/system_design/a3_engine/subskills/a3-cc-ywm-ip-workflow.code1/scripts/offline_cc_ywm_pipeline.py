#!/usr/bin/env python3
"""存储业务面端口互联 + 资源表 → 存储业务面地址规划.xlsx（L2/L3 自动识别，确定性，无 LLM）。"""
...
from cc_ywm_ip_allocate import (  # noqa: E402
    allocate_osp_ips,
    allocate_osa800_ips,
    find_available_ip_vlan_range,
    parse_ip_vlan_pool,
)
from cc_ywm_segment_rules import (  # noqa: E402
...
def plan_leaf_gateways(
    *,
    ...
    from cc_ywm_segment_rules import switch_gateways_to_markdown
...
def _write_outputs(
    ...
    *,
    ...
    with pd.ExcelWriter(out_addr, engine="openpyxl") as writer:
        if osp_df is not None and not osp_df.empty:
            osp_df.to_excel(
                writer, sheet_name=f"{WEB_NETWORK_TYPE_NAME}地址规划", index=False
            )
        if osa800_df is not None and not osa800_df.empty:
            sheet_a800 = (
                f"{WEB_NETWORK_TYPE_NAME}地址规划(OSA800)"
                if osp_df is not None and not osp_df.empty
                else f"{WEB_NETWORK_TYPE_NAME}地址规划"
            )
            osa800_df.to_excel(writer, sheet_name=sheet_a800, index=False)

    out_ip = run_dir / f"A3{WEB_NETWORK_TYPE_NAME}IP地址规划.xlsx"
    with pd.ExcelWriter(out_ip, engine="openpyxl") as writer:
        if osp_df is not None and not osp_df.empty:
            osp_df.to_excel(
                writer, sheet_name=f"{WEB_NETWORK_TYPE_NAME}IP地址规划", index=False
            )
        if osa800_df is not None and not osa800_df.empty:
            sheet_a800 = (
                f"{WEB_NETWORK_TYPE_NAME}IP地址规划(OSA800)"
                if osp_df is not None and not osp_df.empty
...
def generate_cc_ywm_ip_address_xlsx(
    *,
    ...
    if layer is None or str(layer).strip().lower() in ("", "auto"):
        layer, gateway_pos, layer_rule = detect_cc_ywm_layer(resource, sheet_res_index)
    else:
        layer = str(layer).upper()
        if layer not in ("L2", "L3"):
            raise ValueError("layer must be L2, L3, or auto")
        _, gateway_pos, layer_rule = detect_cc_ywm_layer(resource, sheet_res_index)
    ...
    osp_df_raw, osa800_df_raw = get_node_info(path_connect, sheet_connect)
    scenario = classify_device_scenario(osp_df_raw, osa800_df_raw)
    ...
    mask = str(fuzzy_column_value(resource_row, "最小规划掩码"))
    gateway_info = str(resource_row.get("网关地址*", "")).strip()
    vlan_pool = str(resource_row.get("VLAN*", "")).strip()
    net_mode = get_net_mode(resource, sheet_res_index)
    ...
    osp_result: Optional[pd.DataFrame] = None
    osa800_result: Optional[pd.DataFrame] = None

    if scenario == "osp_only":
        osp_result = allocate_osp_ips(
            osp_original_df=osp_df_raw,
            switch_network_segment_info=leaf_gateways,
            ip_pool=ip_pool,
        )
    elif scenario == "osa800_only":
        ip_vlan_dict = parse_ip_vlan_pool(ip_pool, vlan_pool)
        osa800_result = allocate_osa800_ips(
            osa800_original_df=osa800_df_raw,
            ip_vlan_dict=ip_vlan_dict,
            mask=mask,
            gateway_info=gateway_info,
            layer=layer,
            net_mode=net_mode,
            mixed_with_osp=False,
        )
    else:
        osp_result = allocate_osp_ips(
            osp_original_df=osp_df_raw,
        ...
        new_ip_vlan = find_available_ip_vlan_range(
            ip_pool, vlan_pool, leaf_gateways
        )
        osa800_result = allocate_osa800_ips(
            osa800_original_df=osa800_df_raw,
            ip_vlan_dict=new_ip_vlan,
            mask=mask,
            gateway_info=gateway_info,
            layer=layer,
            net_mode=net_mode,
    ...
    if (osp_result is None or osp_result.empty) and (
        osa800_result is None or osa800_result.empty
    ):
    ...
    out_ip = _write_outputs(
        ...
        osp_df=osp_result,
        osa800_df=osa800_result,
        scenario=scenario,
    )

    source_script = (
        "a3_cc_ywm_ip_address.py"
        if layer == "L3"
        else "a3_l2_cc_ywm_ip_address.py"
    )
    meta = {
        "layer": layer,
        ...
        "osp_node_count": len(osp_df_raw),
        "osa800_row_count": len(osa800_df_raw),
    }
    ...
    from _runtime_shared.network_access_plan import (
        AccessPlanSpec,
        PLANE_SPECS,
        build_vlan_map_storage_ywm,
        emit_merged_access_plan,
        filter_assignable_address_rows,
    )

    addr_parts = [
        filter_assignable_address_rows(x)
        for x in (osp_result, osa800_result)
        if x is not None and not x.empty
    ]
    addr_concat = (
        pd.concat(addr_parts, ignore_index=True) if addr_parts else pd.DataFrame()
    )
    base_spec = PLANE_SPECS[NET_PLANE]
    emit_merged_access_plan(
        run_dir,
        connect_path=path_connect,
        connect_sheet=sheet_connect,
        address_df=addr_concat,
        spec=AccessPlanSpec(
            base_spec.network_plane,
            base_spec.vlan_column,
            port_type="Trunk" if scenario in ("osa800_only", "mixed") else "Access",
            label=base_spec.label,
            pvid_from_vlan=scenario in ("osp_only", "mixed"),
            merge_plane=base_spec.merge_plane,
        ),
        search_dirs=[out_dir],
        device_vlan_map=build_vlan_map_storage_ywm(osp_result, osa800_result, NET_PLANE),
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(
        description="Offline A3 存储业务面 IP planning（自动识别 L2/L3，统一指令）"
    )
    p.add_argument(
        "--force-layer",
        default=None,
    ...
    try:
        out = generate_cc_ywm_ip_address_xlsx(
