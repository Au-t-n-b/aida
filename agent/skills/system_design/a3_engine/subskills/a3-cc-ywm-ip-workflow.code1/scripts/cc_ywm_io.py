"""读取存储业务面端口互联表与资源表（对齐 a3_*_cc_ywm_ip_address.py）。"""
...
OSP_DEVICE_PATTERN = "OSP9950|OSP9550"
NET_PLANE = "存储业务面"
WEB_NETWORK_TYPE_NAME = "存储业务面"
SHEET_CONNECT_PREFERRED = "存储业务面端口互联"
...
def _osp_pattern() -> str:
    return OSP_DEVICE_PATTERN
...
def resolve_connect_sheet(path: Path, sheet: object) -> object:
    ...
    if spec is not None:
        ...
        raise ValueError(
            f"指定 sheet {spec!r} 无法读取存储业务面互联数据。"
...
def get_node_info(path: Path, sheet: object) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    对齐 a3_cc_ywm_ip_address.get_node_info：
    返回 (osp_equip_df, osa800_equip_df)。
    """
    ...
    osp_equip = equip[equip["起始端设备"].str.contains(_osp_pattern(), na=False)].copy()
    osa800_equip = equip[equip["起始端设备"].str.contains("OSA800", na=False)].copy()
    ...
    if not osp_equip.empty:
        osp_equip["节点名称"] = (
            osp_equip["起始端设备"] + "-" + osp_equip["起始端接口名称"].map(extract_node)
        )
        osp_equip = osp_equip[["节点名称", "目的端设备"]].copy()
    else:
        osp_equip = pd.DataFrame(columns=["节点名称", "目的端设备"])

    return osp_equip, osa800_equip


def get_switch_to_nodes(path: Path, sheet: object) -> Dict[str, List[str]]:
    """
    对齐 get_switch_info：按起始端设备去重后统计每 Leaf 设备数；
    IP 规划需求 = len(devices) × 8。
    """
    df = read_connectivity_raw(path, sheet)
    work = df.iloc[:, [0, df.columns[-1]]].copy()
    work.columns = ["起始端设备", "leaf交换机"]
    pattern = _storage_pattern()
    work = work[work["起始端设备"].str.contains(pattern, na=False)]
...
def switch_demand_markdown(switch_to_nodes: Dict[str, List[str]], header: str) -> str:
    from cc_ywm_segment_rules import ip_demand_for_nodes
...
def detect_cc_ywm_layer(path: Path, sheet_index: int = 0) -> Tuple[str, str, str]:
    """
    自动识别 L2/L3（与 main_flow / a3-cc-glm-ip-workflow 一致）：

    - 网关位置* == LEAF → L3 → a3_cc_ywm_ip_address.py
    - 网关位置* == SPINE（或其它非 LEAF）→ L2 → a3_l2_cc_ywm_ip_address.py
    """
    ...
    if gw == "LEAF":
        return (
            "L3",
            gw,
            "网关位置*=LEAF → LEAF接入三层 → 存储业务面地址规划_L3 → a3_cc_ywm_ip_address.py",
        )
    return (
        "L2",
        gw,
        f"网关位置*={gw!r} → LEAF接入二层 → 存储业务面地址规划_L2 → a3_l2_cc_ywm_ip_address.py",
    )
...
def get_net_mode(path: Path, sheet_index: int = 0) -> str:
    """对齐 a3_l2_cc_ywm_ip_address.get_net_mode（仅 L2 OSA800 分支使用）。"""
    df = read_network_resource_df(path, sheet_index)
    rows = df[df["网络平面"].astype(str) == NET_PLANE]
    if rows.empty:
        return ""
    row = rows.iloc[0]
    if "组网场景" in row.index:
        return str(row.get("组网场景", "") or "").strip()
    return ""
...
def classify_device_scenario(
    osp_df: pd.DataFrame, osa800_df: pd.DataFrame
) -> str:
    """对齐 a3_cc_ywm_ip_address_generate 三分支。"""
    has_osp = not osp_df.empty
    has_osa800 = not osa800_df.empty
    if has_osp and not has_osa800:
        return "osp_only"
    if not has_osp and has_osa800:
        return "osa800_only"
    if has_osp and has_osa800:
        return "mixed"
    raise ValueError("端口互联表中未找到 OSP9950/OSP9550 或 OSA800 存储设备")
...
def read_leaf_spine_mapping(
    ...
    if spec is None:
        ...
        if SHEET_COMPUTE_MANAGE_FALLBACK in xf.sheet_names:
            spec = SHEET_COMPUTE_MANAGE_FALLBACK
        else:
            spec = resolve_connect_sheet(path, sheet)
    raw = pd.read_excel(path, sheet_name=spec, header=0, dtype=str).fillna("")
    header_idx = None
