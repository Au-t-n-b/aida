#!/usr/bin/env python3
"""Validate inputs for A3 存储带外管理地址规划 offline pipeline."""
...
def validate_007(path_007: Path, sheet: object) -> list[str]:
    ...
    if data.shape[1] < 2:
        return ["007: need at least 2 columns (存储设备, 接入交换机)"]
    ...
    try:
        ...
        leaf_counts = count_storage_ips_by_leaf(str(path_007), sheet)
        if leaf_counts.empty:
            errors.append("007: no storage device rows after filtering leaf/spine")
        else:
            print("007: leaf/ip counts:")
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Validate A3 存储带外管理地址规划 inputs")
