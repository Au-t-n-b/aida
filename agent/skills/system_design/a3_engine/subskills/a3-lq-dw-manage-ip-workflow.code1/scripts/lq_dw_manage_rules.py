...
业务来源：
  - L2：src/manage_agent/sub_agents/LLD_IP/a3_lq_dw_manage_ip_address.py
  - L3：src/manage_agent/sub_agents/LLD_IP/a3_l3_lq_dw_manage_ip_address.py
"""
...
NET_PLANE_RESOURCE = "灵衢带外管理面"
SHEET_DEFAULT = "灵衢带外管理面端口互联"
LQ_DEVICE_PATTERN = re.compile(r"-sp\d+-", re.IGNORECASE)
...
def get_lq_data(excel_path: str, sheet_name: object) -> Dict[str, List[str]]:
    """接入交换机 -> 灵衢设备列表；只保留设备名含 -sp数字- 的行。"""
    ...
    result_df = result_df[result_df["网络设备"].astype(str).str.contains(LQ_DEVICE_PATTERN, na=False)]
    result_df = result_df.drop_duplicates()
    return result_df.groupby("接入交换机", sort=False)["网络设备"].apply(list).to_dict()


def count_lq_by_leaf(excel_path: str, sheet_name: object) -> pd.DataFrame:
    df_all = read_sheet_raw(excel_path, sheet_name)
    header_idx = find_header_row(df_all)
    df = df_all.iloc[header_idx + 1 :].reset_index(drop=True)
    df = df.rename(columns={df.columns[0]: "网络设备", df.columns[-1]: "接入交换机"})
    df = df[df["网络设备"].astype(str).str.contains(LQ_DEVICE_PATTERN, na=False)]
...
def lq_port_map(structured_df: pd.DataFrame) -> Dict[str, object]:
    port_df = structured_df[["起始端信息-设备命名", "起始端信息-接口信息"]].drop_duplicates()
    return port_df.set_index("起始端信息-设备命名")["起始端信息-接口信息"].to_dict()
...
def allocate_lq_dw_addresses(
    switch_gateways: Sequence[SwitchGateway],
    switch_to_lq: Dict[str, List[str]],
    ...
    for net_seg, switches in network_segment_to_switches.items():
        all_lq_device: List[str] = []
        for switch in switches:
            all_lq_device.extend(switch_to_lq.get(switch, []))
        if not all_lq_device:
            continue
        ...
        if len(usable_ips) < len(all_lq_device):
            raise ValueError(f"网段 {net_seg} 中可用 IP 不足，请检查！")

        for lq_device, ip in zip(all_lq_device, usable_ips):
            rows.append(
                {
                    "设备名称": lq_device,
                    "带外管理地址": ip,
                    "带外管理掩码": gateway_info.mask,
                    "带外管理网关": f"{gateway_ip}",
                    "带外管理VLAN": gateway_info.vlan,
                    "接口名称": port_dict.get(lq_device, None),
                }
            )
