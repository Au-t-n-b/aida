#!/usr/bin/env python3
"""007 + 项目信息收集表 -> output/run_*/A3灵衢带外管理地址规划.xlsx。"""
...
from lq_dw_manage_rules import (
    NET_PLANE_RESOURCE,
    SHEET_DEFAULT,
    allocate_lq_dw_addresses,
    count_lq_by_leaf,
...
def generate_lq_dw_manage_xlsx(
    *,
    mode: str,
    ...
    leaf_counts = count_lq_by_leaf(str(path_007), sheet_obj)
    switch_to_lq = get_lq_data(str(path_007), sheet_obj)
    structured = read_structured_007(str(path_007), sheet_obj)
    port_dict = lq_port_map(structured)
    ...
    address_df = allocate_lq_dw_addresses(leaf_gateways, switch_to_lq, port_dict, net_resource.ip_pool)

    run_dir = out_dir.resolve()  # 产物直接落在 Output（不再加 run_<时间戳> 子目录）
    run_dir.mkdir(parents=True, exist_ok=True)
    out_file = run_dir / "A3灵衢带外管理地址规划.xlsx"
    with pd.ExcelWriter(out_file, engine="openpyxl") as writer:
        address_df.to_excel(writer, sheet_name="灵衢带外管理地址", index=False)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Offline A3 灵衢带外管理地址规划（L2/L3 自动分支）")
    parser.add_argument(
        "--mode",
        choices=["auto", "l2", "l3"],
        default="auto",
    ...
    try:
        out_file = generate_lq_dw_manage_xlsx(
            mode=args.mode,
