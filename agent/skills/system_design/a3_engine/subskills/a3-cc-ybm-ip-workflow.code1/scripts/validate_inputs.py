...
def validate_connect(path: Path, sheet: object) -> List[str]:
    ...
    try:
        from cc_ybm_io import find_header_row
    ...
    data = raw.iloc[h + 1 :]
    if data.empty:
        return ["connect: no data rows after header"]
    if data.shape[1] < 3:
...
def validate_resource(path: Path, sheet_index: int) -> List[str]:
    errors: List[str] = []
    try:
        from cc_ybm_io import read_network_resource_df
    ...
    if not errors:
        try:
            layer, gw_pos, rule = detect_cc_ybm_layer(path, sheet_index)
            print(
...
def main() -> int:
    p = argparse.ArgumentParser(description="Validate A3 存储样本面 IP inputs")
