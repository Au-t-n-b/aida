#!/usr/bin/env python3
"""校验 YWM（业务面）离线流水线输入 — 可按 --network-plane 指定网络平面。"""
...
from ywm_io_utils import autodetect_007_in_cwd, autodetect_resource_in_cwd, require_under_cwd
...
def validate_007(path: Path, sheet_name: str, server_substring: str) -> List[str]:
    ...
    if server_substring:
        ...
        if filtered.empty:
            errors.append(
                f"007: after filter server_substring={server_substring!r}, no rows left in first column"
            )
            return errors
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Validate inputs for offline YWM pipeline")
    ...
    p.add_argument("--network-plane", default="计算业务面", help="资源表中要匹配的网络平面名称")
    ...
    all_err: List[str] = []
    all_err.extend(validate_007(args.path_007, args.sheet007, args.server_substring or ""))
