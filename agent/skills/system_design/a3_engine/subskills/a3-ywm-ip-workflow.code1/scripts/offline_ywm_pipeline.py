#!/usr/bin/env python3
"""007 + 项目信息收集表 → 业务面 IP 地址规划 Excel（确定性，无 LLM）。i2：单网段；i3：接入交换机（末列）一机一网段。"""
...
_A3_YWM_DIR = _THIS_FILE.parent.parent
_REPO_ROOT = _A3_YWM_DIR.parent
...
from ywm_i2_allocation import allocate_flat_ywm_ips
from ywm_io_utils import (
    SHEET_007_DEFAULT,
    SHEET_RES_DEFAULT,
...
def _format_user_facing_error(e: Exception) -> str:
    ...
    # Common actionable hints by substring.
    hints: List[str] = []
...
def _summarize_insufficient_markers(df: "pd.DataFrame") -> Dict[str, List[str]]:
    """
    Return {marker: [device_names...]} for any insufficiency markers present in dataframe.
    """
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    p = argparse.ArgumentParser(description="Offline A3 YWM (业务面) IP planning — i2 flat / i3 per-leaf")
    p.add_argument(
        "--topology",
        choices=("i2", "i3"),
        required=True,
    ...
    p.add_argument(
        "--skip-prompt-check",
        action="store_true",
        help="跳过 a3_ywm 下 i2/i3 prompt .py 的短语对齐检查",
    )
