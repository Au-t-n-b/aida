---
name: a3-ywm-ip-workflow
description: 离线 A3 业务面（YWM）IP 规划：007 + 项目信息收集表 → output/run_*/{网络平面}_地址规划.xlsx（确定性，无 LLM）。i2 单网段 / i3 按接入交换机一机一网段。
disable-model-invocation: true
metadata:
  entrypoint: a3_ywm/scripts/offline_ywm_pipeline.py
---
> 本文件是 a3-intelligent-network-opening 主 Skill 的子指令，由 [subskills/SKILL.md](../SKILL.md) 编排调用。

## Purpose

离线生成「业务面（YWM）」地址分配结果：输入两份 Excel，输出一份结果 Excel。该流程为**确定性计算**（不依赖 LLM）。

- **i2**：所有服务器共用 1 个 IPv4 子网，按去重后的顺序依次分配
- **i3**：按 007 末列「接入交换机」一机一网段（每台交换机独立子网）

**网络平面与 skill 名称**：文档标题里的「业务面（YWM）」表示本 skill 的流程归属；**本次规划的是哪一类网络**完全由资源表「网络平面」列与参数 `--network-plane` 决定（输出文件名与列名中的 `{网络平面}` 均取自该参数）。`offline_ywm_pipeline.py` 与 `validate_ywm_inputs.py` 的默认值统一为 `计算业务面`，与「业务面」语义一致；**该字符串必须与资源表中对应行的「网络平面」单元格完全一致**。若你使用的资源表（含仓库内旧版 `reference` 示例）里该行仍为其它名称，请在命令中显式传入与表内一致的 `--network-plane`，或先在表中把该行改名为 `计算业务面` 并补齐地址池等字段。
...
`python a3_ywm/scripts/offline_ywm_pipeline.py --topology i2|i3 [--network-plane NAME] [--007 PATH] [--resource PATH] [--out-dir output]`
...
在包含两份 Excel（007 与资源表）的目录执行。下列命令中的 `--network-plane "计算业务面"` 与**脚本默认值**一致；请确认资源表「网络平面」列中存在该取值的一行（否则改为表中实际字符串）。

- i3（推荐，按接入交换机一机一网段）：

`python a3_ywm/scripts/offline_ywm_pipeline.py --topology i3 --network-plane "计算业务面" --out-dir output --skip-prompt-check`

- i2（单网段，所有服务器共用一个子网）：

`python a3_ywm/scripts/offline_ywm_pipeline.py --topology i2 --network-plane "计算业务面" --out-dir output --skip-prompt-check`
...
`python a3_ywm/scripts/offline_ywm_pipeline.py --topology i3 --sheet007 1 --server-substring "D01" --network-plane "计算业务面" --out-dir output --skip-prompt-check`

## Directory Contract

本 skill 运行时假设目录结构如下（与当前仓库一致）：

- `a3_ywm/scripts/offline_ywm_pipeline.py`：主入口
- `a3_ywm/scripts/*.py`：YWM 离线逻辑
- `scripts/dw_manage_segment_rules.py`：通用网段切分/网关/VLAN 规则（供 i3 与部分 i2 复用）

## Parameters

- `--topology i2|i3`：必选，选择二层单网段或三层一机一网段
- `--network-plane NAME`：须与资源表「网络平面」列某一行的取值**完全一致**。脚本与 `validate_ywm_inputs.py` 的默认值为 `计算业务面`；若表中无此行名，请传入表中真实字符串（含沿用旧示例表时）。
...
`python a3_ywm/scripts/validate_ywm_inputs.py --network-plane "计算业务面"`

其中 `--network-plane` 须与你要校验的资源表行一致（与 `offline_ywm_pipeline.py` 默认值一致时为 `计算业务面`）。
