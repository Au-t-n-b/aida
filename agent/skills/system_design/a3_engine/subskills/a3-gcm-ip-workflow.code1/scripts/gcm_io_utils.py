"""007 / 资源表读取：管存面（GCM）离线流程与 dw 流程共用模式，增加服务器名过滤。"""
...
# Align main_flow.file_configs「计算管存面地址规划」→ 计算管存面端口互联
    ...
    def _read_one(sheet: object) -> Dict[str, List[str]]:
        ...
        if not out:
            raise ValueError(
                ...
                "  - 建议: 检查该 sheet 是否为管存面/计算服务器连线页；或用 --sheet007 指定正确 sheet。"
            )
        return out
