"""i2（二层聚合）语义：整网单一子网内，按地址池裁剪可用主机地址并顺序分配给全部节点。"""
...
from segment_split import network_safe_for_host_enum, pool_fits_in_network  # noqa: E402


def allocate_flat_ybm_ips(
    ...
    *,
    ...
    """
    :param equipment_names_ordered: 已去重后的设备名列表（顺序即分配顺序）
    :param plane_label: 用于输出列名前缀（如 「计算参数面」→ 「计算参数面地址」）
    :return: (结果表, 实际 IPv4Network)
    """
    ...
    gateway_ip = ipaddress.IPv4Address(determine_gateway(str(network), gateway_config))
