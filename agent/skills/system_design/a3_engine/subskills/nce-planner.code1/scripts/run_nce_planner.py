#!/usr/bin/env python3
"""Offline NCE deployment planner: 007 + resource table -> A3NCE部署规划.xlsx."""
...
from nce_planner import PlannerInputs, run_planner, write_nce_excel
from nce_topology_parser import DEFAULT_SHEET

DEFAULT_OUT = "A3NCE部署规划.xlsx"
...
def _auto_topology() -> Path:
    candidates = [
        path
        for path in _list_excel_in_cwd()
        if "007" in path.name or "端口连线" in path.name or "端口互联" in path.name
    ]
    return _pick(candidates, "007 topology")
...
def _auto_resource() -> Path:
    candidates = [
        path
        for path in _list_excel_in_cwd()
        if "项目信息收集" in path.name or "资源" in path.name
    ]
    return _pick(candidates, "resource table")


def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Offline NCE deployment planner")
    ...
    try:
        ...
        written = write_nce_excel(result, str(_resolve_local(out_path)))
    ...
    print(
        "  rows: "
        f"NCEFB nodes={result.fb_node_rows}, NCEFB floating={result.fb_floating_rows}, "
        f"NCEFI nodes={result.fi_node_rows}, NCEFI floating={result.fi_floating_rows}"
    )
    print(f"  switch_mlag_flag: {result.switch_mlag_flag}")

    if args.print_markdown:
        print()
        print(result.final_df.to_markdown(index=False))
