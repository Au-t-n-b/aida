...
`report_gen_run` 在本地联调时使用 `输入文件/本地工勘报告.pdf` 作为演示撰写源文件。
...
| 运行时落点 | `{AIDA_BUSINESS_ROOT}/project/交付作业/智慧工勘/输入文件/本地工勘报告.pdf`（init/reset 从项目资产复制） |
...
- `--dest-workspace` 会同时复制到当前智慧工勘工作区
...
```bash
# 清 输出结果/解析结果/勘测图片文件夹，保留 org-assets（底表+风险库）
