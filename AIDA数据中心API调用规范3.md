...
> **与 v2 的关键差异**：新增 `contractType`（合同类型）必填字段与 DB CHECK 约束；新增 `deliveryTraits`（交付场景）JSONB 字段支持多选；`DELETE /roles/{roleId}` 正式实装三重安全保护（存在性/内置角色/用户绑定）；项目创建权限放开（任何登录用户均可创建，审批环节兜底）；文件上传中文文件名编码自动修复（GBK/UTF-8 mojibake recovery）；接口总数 53 个。
...
- [附录 A 相对 v2 的变更对照](#附录-a-相对-v2-的变更对照)
...
| 字段风格 | 请求/响应字段为 **camelCase**（如 `projectId`、`fileStage`、`folderSubPath`、`contractType`、`deliveryTraits`） |
...
> **注意**：当请求 Body 字段校验失败（缺少必填字段、枚举值不合法等），统一返回 HTTP 400，`code=400`，`message="参数错误"`，`data` 中包含 Pydantic 校验错误详情数组。
...
文件上传接口内置了编码自动修复：当 multipart 上传的中文文件名出现 GBK/UTF-8 mojibake（如 `æ\x96\x87ä»¶å\x90\x8d`），服务端会自动恢复为正确的 UTF-8 文件名。调用方无需特殊处理。

---

## 第 2 章 功能模块总览

| # | 功能模块 | 路由前缀 | 接口数 | 主要职责 | 与业务数据的关系 |
| --- | --- | --- | --- | --- | --- |
| 1 | 认证 auth | `/api/v1/users`、`/api/v1/auth` | 5 | 注册、登录（平台/外部）、当前用户、删除用户 | 取 token |
| 2 | 项目 project | `/api/v1/projects`、`/api/v1/users/{username}/projects` | 15 | 项目 CRUD、审批、归档、成员、目录树、按用户查项目、运行上下文 | 项目空间与目录树入口 |
| 3 | 文件 file | `/api/v1/files`、`/api/v1/directories`、`/api/v1/storage` | 9 | 业务文件增删改查、目录增删改、存储统计（**全语义寻址**） | **业务数据面核心** |
| 4 | 组织资产 org-asset | `/api/v1/org-assets` | 3 | 公共/主数据只读、元数据维护 | 参考/主数据 |
| 5 | 数据血缘 lineage | `/api/v1/file-lineage` | 1 | 文件血缘查询 | 数据追溯 |
| 6 | Skill 中心 skill | `/api/v1/skills` | 11 | Skill 注册/发布/包文件/融合 | Skill 文件管理 |
| 7 | 配置中心-人员 user-mgmt | `/api/v1/user-mgmt` | 5 | 人员 CRUD | 独立业务数据 |
| 8 | 配置中心-角色权限 rbac | `/api/v1/roles`、`/api/v1/permissions` | 8 | 角色、权限点、映射 | 权限 |
| 9 | 配置中心-模型 model-config | `/api/v1/model-configs` | 4 | 模型配置 CRUD | 模型 |
| 10 | 运维中心-审计 audit | `/api/v1/audit-logs` | 1 | 审计日志查询 | 操作留痕 |

> **合计 53 个接口**（含健康检查）。对 skill / 业务开发最常用的是模块 3/4/5/6（文件、组织资产、血缘、Skill）。

---

## 第 3 章 语义定位与数据发现

> 文件/目录的读写**不需要先取 `nodeId`/`fileId` 再操作**，而是直接用业务语义参数定位。目录树接口仍保留，但仅用于**浏览展示**，不再是操作的前置必需步骤。
...
TOKEN=$(curl -s -X POST "$BASE/api/v1/users/login" \
  ...
  -d '{"username":"zhangsan","password":"xxx"}' | jq -r '.data.token')

# 2) 创建项目（contractType 必填）
curl -X POST "$BASE/api/v1/projects" \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
  -d '{
    "projectName":"深圳数据中心一期",
    "contractType":"标准合同",
    "customerName":"XX科技",
    "tdUsername":"zhangsan",
    "deliveryTraits":["新建","液冷"]
  }'
#    → data.projectId（UUID32，后续所有接口用这个标识项目）

# 3) 管理员审批通过
curl -X POST "$BASE/api/v1/projects/$PROJECT_UUID/approve" \
...
> 合同类型枚举：`预销售合同` / `标准合同`（DB CHECK 约束限定，新建时必填）。
> 交付场景：`deliveryTraits` 为 JSONB 数组，可存多个值（如 `["新建", "扩容", "液冷"]`），包括产品代际、制冷方式、训推类型、Pod 形态等交付特点信息。
> **项目可见性**：普通用户只能看到自己参与的项目（creator/td/pd/pcm 或在 `user_project_roles` 中有该项目角色）；列全部项目（`GET /projects`）已收紧为**仅管理员**。

#### POST /api/v1/projects — 创建项目
- **功能**：创建一个新项目，初始状态为「待审批」；同时按目录骨架模板生成该项目目录树（本期模块同步建物理目录，暂不模块仅建库行）。可在创建时一并指定关键角色与初始成员。
- **鉴权**：必填（仅需登录）——**任何登录用户均可创建项目**；新建后为待审批状态，由审批环节把关。
- **入参**：

| 参数 | 位置 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- | --- |
| projectName | Body | string | 是 | 项目名称（不可重复） |
| contractType | Body | string | **是** | 合同类型，枚举：`预销售合同` / `标准合同`（DB CHECK 约束限定，传其他值返回 400） |
| bidCode | Body | string | 否 | 投标编码 |
| customerName | Body | string | 否 | 客户名称 |
| projectCode | Body | string | 否 | 项目编码 |
| tdUsername | Body | string | 否 | TD（技术交付）账号用户名（必须是系统已有用户） |
| pdUsername | Body | string | 否 | PD（项目总监）账号用户名（必须是系统已有用户） |
| pcmUsername | Body | string | 否 | PCM 账号用户名（必须是系统已有用户） |
| stage | Body | string | 否 | 项目阶段 |
| progress | Body | int | 否 | 项目进度百分比（0~100），默认 0 |
| risk | Body | string | 否 | 风险等级/描述，默认 `low` |
| description | Body | string | 否 | 项目描述 |
| deliveryTraits | Body | array | 否 | 交付场景（JSONB 数组），如 `["新建", "扩容", "液冷"]` |
| members | Body | array | 否 | 初始成员列表，元素 `{ userId: int, roleId: int }` |
...
- **错误**：2002 项目名已存在；400 缺少必填字段或 `contractType` 值不在枚举范围；1004 `tdUsername`/`pdUsername`/`pcmUsername` 用户不存在。
...
| status | string | 项目状态 |
| contractType | string | 合同类型（`预销售合同`/`标准合同`，旧项目可能为空） |
| stage | string | 项目阶段 |
| progress | int | 进度百分比 |
| risk | string | 风险等级 |
| description | string | 项目描述 |
| deliveryTraits | array | 交付场景（JSONB 数组，如 `["新建", "扩容"]`） |
...
| contractType | Body | string | 否 | 合同类型：`预销售合同` / `标准合同` |
...
| description | Body | string | 否 | 项目描述 |
| deliveryTraits | Body | array | 否 | 交付场景（JSONB 数组） |

- **出参**：更新后的项目对象，字段同「GET /projects」列表元素。
- **错误**：2002 项目名已存在；1004 用户名不存在。
...
- **功能**：把文件上传到「指定模块+阶段+子目录」对应目录。`folderSubPath` 指向的子目录不存在时**自动逐级创建**；同目录已有同名文件则复用其节点并更新内容（可安全重试）。中文文件名自动修复编码问题。
...
- **功能**：删除一个角色（同时清空其角色-权限映射）。**三重安全保护**：角色不存在返回 404；内置角色禁止删除返回 403；仍有用户绑定的角色禁止删除返回 400。
- **鉴权**：`role:delete`。
- **入参**：
...
- **出参**：`{ deleted: bool }`。
- **错误**：404 角色不存在；403 内置角色禁止删除（ADMIN/TD/PD/PCM/TL/SURVEYOR）；400 该角色仍有用户绑定，无法删除。
...
> 本表汇总 v3 相对 v2（《AIDA 数据中心 API 调用规范 2》v3.0）的接口变更，便于存量调用方迁移。

### A.1 项目模块变更

| 变更 | 说明 |
| --- | --- |
| **新增字段** `contractType` | 项目新建**必填**（`预销售合同` / `标准合同`，DB CHECK 约束）；更新可选；列表/详情/更新返回均包含 |
| **新增字段** `deliveryTraits` | 项目交付场景（JSONB 数组，如 `["新建","扩容"]`），新建可选；列表/详情/更新返回均包含 |
| **变更** `POST /projects` 鉴权 | 由 `project:create` 权限门控改为**仅需登录**（任何登录用户可创建，审批环节兜底） |
| **实装** `DELETE /roles/{roleId}` | v2 中 405 Method Not Allowed → v3 正式实装，三重安全保护（存在性/内置角色/用户绑定） |

### A.2 文件模块增强

| 变更 | 说明 |
| --- | --- |
| **中文文件名编码修复** | `POST /files/upload` 内置 GBK/UTF-8 mojibake 自动恢复，调用方无需特殊处理 |
| **参数校验错误码统一** | 所有接口缺少必填字段或枚举值不合法时统一返回 HTTP 400（之前部分接口返回 422） |

### A.3 前端功能变更（不影响 API 契约）

| 变更 | 说明 |
| --- | --- |
| 导航结构调整 | 新增「项目管理」导航组（含项目列表 + 项目审批），原「配置中心·项目配置」移入 |
| 项目创建弹窗 | 新增合同类型下拉（必选） |
| 项目编辑弹窗 | 新增合同类型下拉（可选） |
| 项目详情抽屉 | 新增合同类型显示 |
| 项目操作 | 删除 → 归档（仅 APPROVED）；新增「重新提交」（仅 REJECTED） |
...
| 版本 | 日期 | 说明 |
| --- | --- | --- |
| v4.0 | 2026-06-15 | 新增 contractType 必填字段与 DB CHECK 约束；新增 deliveryTraits JSONB 字段；DELETE /roles/{roleId} 实装三重安全保护；项目创建权限放开（登录即可创建）；文件上传中文文件名编码自动修复；参数校验错误统一 400；接口总数 53 个；附录 A 给出 v2→v3 变更对照 |
| v3.0 | 2026-06-12 | 文件/目录/组织资产全面语义寻址（去 fileId/nodeId 入参）；下载改 GET 查询参数；新增文件/目录重命名；项目列表按用户隔离；新增外部登录与运行上下文接口 |
